...
namespace rayzngames
{
	...
	public class BicycleVehicle : MonoBehaviour
	{
		...
		[SerializeField]public float minSteeringAngle = 5f;
		...
		[Range(0.001f, 1f)][SerializeField] public float turnSmoothing;

		[Space(20)]
		[Header("Lean")]
		[Space(5)]
		[Tooltip("Defines the maximum leaning angle when turning for this bike")]
		[SerializeField]public float maxLeanAngle = 45f;
		[Tooltip("Sets the Leaning sensitivity (0 - None, 1 - full")]
		[Range(0.001f, 1f)][SerializeField]public float leanSmoothing;
		...
		[Space(10)]
		[SerializeField]public WheelCollider frontWheel;
		[SerializeField]public WheelCollider rearWheel;
		[Space(10)]
		[SerializeField] Transform frontWheelTransform;
		[SerializeField] Transform rearWheelTransform;
		public TrailRenderer frontTrail;
		public TrailRenderer rearTrail;
		[Space(10)]
		...
		public ParticleSystem frontSmoke;
		public ParticleSystem rearSmoke;
		...
		public void ConstrainRotation(bool state)
		{
			if (state == true)
			{
				...
				//rb.constraints = RigidbodyConstraints.FreezePositioY;
		...
		bool SlipRear()
		{
			...
			{
				if (rearFWD > 3f || rearFWD < -3f || rearSideCoeff > 3f || rearSideCoeff < -3f)
				{
					return true;
				}
				else
